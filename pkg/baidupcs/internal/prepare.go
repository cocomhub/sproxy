...
import (
	...
	"github.com/cocomhub/sproxy/pkg/baidupcs/internal/requester/multipartreader"
)
...
// num/page 翻页, 修复 https://github.com/cocomhub/sproxy/pkg/baidupcs/internal/issues/511。
...
func (pcs *BaiduPCS) PrepareLocateDownload(pcspath string) (dataReadCloser io.ReadCloser, pcsError pcserror.Error) {
	pcs.lazyInit()
	bduss := pcs.GetBDUSS()
	// 裁剪说明：原实现 uid==0 时经 tieba 接口自动解析 UID（依赖 baidu-tools 外部包）。
	// 裁剪后改为由调用方 SetUID 注入（接入层从登录态解析）；未注入时无法生成 locatedownload
	// 签名，报错提示（保持 fail-closed，不静默退化）。
	if pcs.uid == 0 {
		return nil, &pcserror.PCSErrInfo{
			Operation: OperationLocateDownload,
			ErrType:   pcserror.ErrTypeOthers,
			Err:       fmt.Errorf("baidupcs: UID 未设置，无法生成 locatedownload 签名（请通过 SetUID 注入）"),
		}
	}
