...
//  2. raw 32B 字节。
//
// 判定顺序：先按 base64 解码（trim 后），解码成功且为 32B 即采用；否则若原始内容
// trim 后恰为 32B 则按 raw 采用；两者都不满足（含文件缺失）返回 error（fail-fast）。
func LoadMasterKeyFromFile(path string) ([]byte, error) {
	...
	// 先试 base64 格式（权威形态：`openssl rand -base64 32`）。
	if b64, derr := base64.StdEncoding.DecodeString(trimmed); derr == nil && len(b64) == 32 {
		return b64, nil
	}
	// 回落 raw 32B 格式。
	if len(trimmed) == 32 {
	...
	return nil, errors.New("accesskey: master key 文件格式非法: 既非 base64 编码的 32B（44 字符含 =），也非 raw 32 字节")
}
