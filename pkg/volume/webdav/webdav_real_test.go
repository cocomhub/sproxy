...
// 协议兼容性（T2，2026-09-18）：
//   - CI 的 Test (ubuntu, +Vault) job 启动 hacdias/webdav 容器（匿名模式，端口 80），
//     本测试检测 WEBDAV_ENDPOINT（默认 http://127.0.0.1:80）可达 → 不可达 t.Skip
...
// realWebDAVEnv 返回真实 WebDAV 服务端配置（WEBDAV_ENDPOINT 默认 127.0.0.1:80）。
func realWebDAVEnv() ClientConfig {
	ep := os.Getenv("WEBDAV_ENDPOINT")
	if ep == "" {
		ep = "http://127.0.0.1:80"
	}
