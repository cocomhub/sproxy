...
// TestRegister_ExistingAccountUnaffected 验证既有账号重复注册不产生新 admin、不清除
// 既有条目，role 不受影响的「防覆盖既有 AK」语义落到 ring 状态断言。
func TestRegister_ExistingAccountUnaffected(t *testing.T) {
	...
	// 回环注册（服务端生成全新 AK——AddRegistration 扫到无 admin → 授 admin）。
	// 既有 user 账号存在但与本次注册不同 AK，AddRegistration 的「全 ring 无 admin」
	// 判定只看 Role——user 不阻止首 admin 诞生。断言原子语义：本次注册 admin=true
	// 且既有 user 账号的角色/条目未被改动。
...
// POST /tunnel 开隧道（内层 ctx 由外层透传 Principal）→ 内层 GET /api/files → 403
// （fail-closed，node 无法借隧道访问文件）。
func TestRegister_TunnelInnerGateNodeDenied(t *testing.T) {
	...
	resp, err := tc.Do(req)
	if err != nil {
		t.Fatalf("node tunnel Do: %v", err)
	}
	defer resp.Body.Close()
	if resp.StatusCode != http.StatusForbidden {
		t.Fatalf("node 隧道内层 GET /api/files status = %d, want 403（fail-closed）", resp.StatusCode)
	}
}

// TestRegister_TunnelInnerGateUserAllowed 验证对向：Role==user 的 AK 开隧道 → 内层
// GET /api/files → 200（user 隧道文件操作可用且落自身租户）。
