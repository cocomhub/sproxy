...
func TestGzipMiddleware_NilLogger(t *testing.T) {
	...
	handler := GzipMiddleware(nil)(inner)
...
func TestBatchRename_Success(t *testing.T) {
	...
	body := []byte("rename me")
	cs := sha256hex(body)
	code, _ := uploadFile(t, url, "oldname.txt", body, map[string]string{"X-File-Checksum": cs})
	if code != 200 {
		t.Fatalf("upload: expected 200, got %d", code)
	}

	reqBody := fmt.Sprintf(`{"operations":[{"from":"oldname.txt","to":"newname.txt","checksum":"%s"}]}`, cs)
	...
	uploadsDir := cfgPtr.Load().UploadsDir
	if _, err := os.Stat(filepath.Join(uploadsDir, "oldname.txt")); !os.IsNotExist(err) {
		t.Fatal("old file should no longer exist")
	}
	if _, err := os.Stat(filepath.Join(uploadsDir, "newname.txt")); err != nil {
		t.Fatal("new file should exist")
	}
}

func TestBatchRename_ContinueOnError(t *testing.T) {
	t.Parallel()
	url, cfgPtr := newTestServerWithAllRoutes(t, nil)

	body := []byte("keep me")
	cs := sha256hex(body)
	code, _ := uploadFile(t, url, "keep.txt", body, map[string]string{"X-File-Checksum": cs})
	if code != 200 {
		t.Fatalf("upload: expected 200, got %d", code)
	}

	reqBody := fmt.Sprintf(`{"operations":[
		{"from":"nonexistent.txt","to":"dest.txt","checksum":"%s"},
		{"from":"keep.txt","to":"moved.txt","checksum":"%s"}
	]}`, cs, cs)
	req, _ := http.NewRequest("POST", url+"/api/batch/rename", strings.NewReader(reqBody))
	req.Header.Set("Content-Type", "application/json")
	resp, err := http.DefaultClient.Do(req)
	...
	if result.Results[0].Success {
		t.Fatalf("expected first operation to fail (nonexistent source), got success: %+v", result.Results[0])
	}
	if !result.Results[1].Success {
		t.Fatalf("expected second operation to succeed (continue-on-error), got: %+v", result.Results[1])
	}
	uploadsDir := cfgPtr.Load().UploadsDir
	if _, err := os.Stat(filepath.Join(uploadsDir, "keep.txt")); !os.IsNotExist(err) {
		t.Fatal("old file 'keep.txt' should no longer exist")
	}
	if _, err := os.Stat(filepath.Join(uploadsDir, "moved.txt")); err != nil {
		t.Fatal("moved file should exist")
	}
}

func TestBatchRename_InvalidPath(t *testing.T) {
	t.Parallel()
	url, _ := newTestServerWithAllRoutes(t, nil)

	reqBody := `{"operations":[{"from":"../escape.txt","to":"safe.txt","checksum":"aa"}]}`
	req, _ := http.NewRequest("POST", url+"/api/batch/rename", strings.NewReader(reqBody))
	req.Header.Set("Content-Type", "application/json")
	resp, err := http.DefaultClient.Do(req)
	if err != nil {
...
func TestSearchFiles_Pagination(t *testing.T) {
	...
	body := []byte("commonprefix-data")
	cs := sha256hex(body)
	for i := range 12 {
		fn := fmt.Sprintf("commonprefix-file-%d.txt", i)
		code, _ := uploadFile(t, url, fn, body, map[string]string{"X-File-Checksum": cs})
		if code != 200 {
			t.Fatalf("upload %s: %d", fn, code)
		}
	...
	t.Run("all_matching", func(t *testing.T) {
		resp, err := http.Get(url + "/api/files/search?q=commonprefix")
		if err != nil {
			t.Fatalf("search: %v", err)
		}
		defer resp.Body.Close()
		...
		if len(result.Files) != 12 {
			t.Fatalf("expected 12 files matching 'commonprefix', got %d", len(result.Files))
		}
	})

	t.Run("no_match", func(t *testing.T) {
		resp, err := http.Get(url + "/api/files/search?q=zzzzz_notfound")
		if err != nil {
			t.Fatalf("search: %v", err)
		}
		defer resp.Body.Close()
...
func TestListFiles_PaginationMore(t *testing.T) {
	t.Parallel()
	url, _, cleanup := newTestServer(t, nil)
	defer cleanup()
	...
	t.Run("offset_1_limit_5", func(t *testing.T) {
		resp, err := http.Get(url + "/api/files?offset=1&limit=5")
		if err != nil {
			t.Fatalf("list: %v", err)
		}
		defer resp.Body.Close()
		...
		if result.Limit != 5 {
			t.Fatalf("limit: expected 5, got %d", result.Limit)
		}
		if len(result.Files) != 5 {
			t.Fatalf("files: expected 5, got %d", len(result.Files))
		}
	})
	...
	t.Run("offset_beyond_end", func(t *testing.T) {
		resp, err := http.Get(url + "/api/files?offset=100&limit=5")
		if err != nil {
			t.Fatalf("list: %v", err)
		}
		defer resp.Body.Close()
		...
		if len(result.Files) != 0 {
			t.Fatalf("expected 0 files (offset beyond total), got %d", len(result.Files))
		}
	})
}
...
func TestListSort_BySizeDesc(t *testing.T) {
	t.Parallel()
	url, _, cleanup := newTestServer(t, nil)
	defer cleanup()
	...
	for _, item := range []struct {
		...
		body := bytes.Repeat([]byte("X"), item.size)
		cs := sha256hex(body)
		code, _ := uploadFile(t, url, item.name, body, map[string]string{"X-File-Checksum": cs})
		if code != 200 {
			t.Fatalf("upload %s: %d", item.name, code)
		}
	}

	resp, err := http.Get(url + "/api/files?sort=size&order=desc")
	if err != nil {
		t.Fatalf("list: %v", err)
	}
	defer resp.Body.Close()
	...
	for _, f := range result.Files {
		if f.Name == "small.txt" || f.Name == "medium.txt" || f.Name == "large.txt" {
			ourFiles = append(ourFiles, f)
		}
	}
	if len(ourFiles) != 3 {
		t.Fatalf("expected 3 uploaded files, got %d", len(ourFiles))
	}
	if ourFiles[0].Name != "large.txt" || ourFiles[1].Name != "medium.txt" || ourFiles[2].Name != "small.txt" {
		t.Fatalf("expected desc by size: [large.txt(1000), medium.txt(100), small.txt(10)], got %+v", ourFiles)
	}
}

func TestListSort_ByTimeAsc(t *testing.T) {
	t.Parallel()
	url, _, cleanup := newTestServer(t, nil)
	defer cleanup()
	...
	resp, err := http.Get(url + "/api/files?sort=time&order=asc")
	if err != nil {
		t.Fatalf("list: %v", err)
	}
	defer resp.Body.Close()
	...
	for _, f := range result.Files {
		if f.Name == "first.txt" || f.Name == "second.txt" || f.Name == "third.txt" {
			order = append(order, f.Name)
		}
	}
	if len(order) != 3 {
		t.Fatalf("expected 3 uploaded files, got %d", len(order))
	}
	if order[0] != "first.txt" || order[1] != "second.txt" || order[2] != "third.txt" {
		t.Fatalf("expected asc by time: [first.txt, second.txt, third.txt], got %v", order)
	}
}
...
func TestGenerateSelfSignedCert(t *testing.T) {
	...
	if _, err := os.Stat(certFile); err != nil {
		t.Fatalf("cert.pem should exist: %v", err)
	}
	if _, err := os.Stat(keyFile); err != nil {
		t.Fatalf("key.pem should exist: %v", err)
	}
	...
	if block == nil || block.Type != "CERTIFICATE" {
		t.Fatal("cert.pem should contain a PEM CERTIFICATE block")
	}
	cert, err := x509.ParseCertificate(block.Bytes)
	if err != nil {
		t.Fatalf("ParseCertificate failed: %v", err)
	}
	if cert.Subject.CommonName != "sproxy.local" {
		t.Fatalf("CommonName: expected 'sproxy.local', got %q", cert.Subject.CommonName)
	}
	if len(cert.Subject.Organization) != 1 || cert.Subject.Organization[0] != "Cocomhub" {
		t.Fatalf("Organization: expected [Cocomhub], got %v", cert.Subject.Organization)
	}
	if len(cert.DNSNames) != 2 || cert.DNSNames[0] != "localhost" || cert.DNSNames[1] != "sproxy.local" {
		t.Fatalf("DNSNames: expected [localhost sproxy.local], got %v", cert.DNSNames)
	}
	if cert.KeyUsage&x509.KeyUsageDigitalSignature == 0 {
		t.Fatal("expected KeyUsageDigitalSignature")
	}
	if cert.KeyUsage&x509.KeyUsageKeyEncipherment == 0 {
		t.Fatal("expected KeyUsageKeyEncipherment")
	}

	t.Run("auto_create_parent_dir", func(t *testing.T) {
		nestedDir := t.TempDir()
		nestedCert := filepath.Join(nestedDir, "sub", "nested", "cert.pem")
		nestedKey := filepath.Join(nestedDir, "sub", "nested", "key.pem")
		if err := GenerateSelfSignedCert(nestedCert, nestedKey); err != nil {
			t.Fatalf("GenerateSelfSignedCert with nested dir failed: %v", err)
		}
		if _, err := os.Stat(nestedCert); err != nil {
			t.Fatalf("nested cert should exist: %v", err)
		}
	})
}
