...
func TestBuildXferClientTLSConfig_DefaultSystemPool(t *testing.T) {
	cfg, err := buildXferClientTLSConfig("", false, "127.0.0.1:9999")
	if err != nil {
...
func TestBuildXferClientTLSConfig_CASetsRootCAs(t *testing.T) {
	...
	cfg, err := buildXferClientTLSConfig(certFile, false, "127.0.0.1:9999")
	if err != nil {
...
func TestBuildXferClientTLSConfig_InsecureLoopback(t *testing.T) {
	for _, addr := range []string{"127.0.0.1:9999", "localhost:9999", "[::1]:9999"} {
		cfg, err := buildXferClientTLSConfig("", true, addr)
		if err != nil {
...
func TestBuildXferClientTLSConfig_InsecureNonLoopbackRejected(t *testing.T) {
	_, err := buildXferClientTLSConfig("", true, "example.com:9999")
	if err == nil {
...
func TestBuildXferClientTLSConfig_CAAndInsecureMutuallyExclusive(t *testing.T) {
	certFile, _ := genTestCertFiles(t)
	_, err := buildXferClientTLSConfig(certFile, true, "127.0.0.1:9999")
	if err == nil {
...
func TestBuildXferClientTLSConfig_CABadFile(t *testing.T) {
	if _, err := buildXferClientTLSConfig(filepath.Join(t.TempDir(), "missing.pem"), false, "127.0.0.1:9999"); err == nil {
	...
	if _, err := buildXferClientTLSConfig(bad, false, "127.0.0.1:9999"); err == nil {
...
func TestBuildXferClientTLSConfig_InsecureBadAddr(t *testing.T) {
	if _, err := buildXferClientTLSConfig("", true, "not-an-addr"); err == nil {
