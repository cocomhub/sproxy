...
// TestHubWSDial_InsecureTLS 验证 mesh.HubWSDial 的 --insecure 接线（B17）：
//   - 自签 wss hub（httptest.NewTLSServer）且未传 --insecure → TLS 握手失败
//     （证书不被信任，保持默认安全行为）；
//   - 传 --insecure → 经 client.InsecureHTTPClient() 跳过证书校验，握手成功。
...
// TestInsecureHTTPClient_UsesInsecureTLS 验证 client.InsecureHTTPClient 的 transport
// 关闭证书校验（InsecureSkipVerify=true），且保留超时（对齐 HubSignaler 长轮询 60s）。
func TestInsecureHTTPClient_UsesInsecureTLS(t *testing.T) {
	hc := client.InsecureHTTPClient()
	if hc == nil {
		t.Fatal("client.InsecureHTTPClient() returned nil")
	}
