...
// restart_signal_windows.go 是 Windows 平台的优雅重启信号桩。
//
// Windows 无 SIGUSR2 投递且 os/exec.ExtraFiles 不支持跨进程套接字继承 → 优雅重启
// 特性天然关闭（registerRestartSignal 空操作、isRestartSignal 恒 false，行为零变化）。
// handleSignalRestart 是符号兼容桩：正常流程不会走到（isRestartSignal 恒 false）。
