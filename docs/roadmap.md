...
| 多卷 | 本地多盘 + 外部后端框架（baidupcs/webdav/s3/sftp/ftp，oss/cos 可经 s3 后端 S3 兼容 endpoint 覆盖）+ 镜像/分层/联邦已落地 | 联邦卷只读（无回写）；本地其它盘未覆盖 | P1 联邦卷回写 + 后端扩展 |
...
| 通知与可观测 | 指标/审计/事件流/追踪骨架已落地，**通知外发已落地**（notify-center 实现中） | 无阈值告警引擎 | P1 阈值告警引擎 |
...
| **外部后端生态薄** | baidupcs/webdav/s3/sftp/ftp 五类 | 缺常见后端（本地其它盘等）；oss/cos 可由 s3 后端经 S3 兼容 endpoint 覆盖（阿里云 OSS/腾讯 COS 均提供 S3 兼容 API，配置 `volumes[] type=s3 + extra.endpoint` 指向对应服务即可，无需专用后端） |
...
| **传输层指标缺失** | 仅 mux 层有指标；TCP/WS/QUIC 指标实现中（metrics-auth 分支） | 跨墙链路劣化难定位是 DPI 限速还是网络抖动 | **已落地**（#497）：`sproxy_xfer_tcp/ws/quic_*` 指标（conns/messages/bytes）入 `/metrics` + [sproxy-dashboard.json](./grafana/sproxy-dashboard.json) 传输层聚合面板。残余：无 |
...
| **P2：tun/tap 内核 VPN** | `sclient mesh up` 升级内核虚拟网卡（整网段透明路由，特权 + 平台集成） | **已落地（P1）**：`pkg/vpn` 接口 + `PlatformProbe` 三态 + `Router` 主循环 + `mesh up --tun --vip` fail-closed 装配（TUNDevice build tag 隔离：Linux tun 骨架 / Windows wintun 标注 / 未支持平台 stub）。残余：P2 真设备打开（Linux ioctl TUNSETIFF / Windows wintun.dll）+ MTU/地址装配；P3 真设备 e2e |
...
| ⑤ | tun/tap 内核 VPN | `pkg/vpn` `TUNDevice`（Open/SetMTU/Addr）+ `PlatformProbe` 三态 + `Router`（读包 → vipTable 选路 → mesh dial → 写包）+ `mesh_up.go` `--tun/--vip` 装配（fail-closed） | **已落地（P1）**：`mesh up --tun` 探测/缺 VIP/未分配明确报错不静默回落；真设备打开 P2、e2e P3 |
...
| ⑧ | S3 complete ETag 校验 | `s3_multipart.go` `normalizeETag`（去引号归一）+ `validateCompleteParts`（PartNumber 范围/重复 400）+ 逐 part 单遍拷贝+哈希（`io.MultiWriter(f, md5.New())`）比对 400 + meta key 一致性 409 + 失败清理半截目标；见 `s3_complete_hardening_test.go`（TestS3Complete_TamperedETag_400_NoLeftover / MetaKeyMismatch_409 / UnquotedETag_200） | **已落地**（#575） |
| ⑨ | S3 complete 配额记账 | `s3_multipart.go` complete 双账本 `TryReserve`（owner Scope + 卷容量池）超限 507 + 失败统一双 `Release` + 成功 `Commit`/覆盖写 `Adjust` 差分（至多一次，Reservation.done CAS）；见 `s3_complete_hardening_test.go`（TestS3Complete_Quota_OwnerExceeded_507 / PoolExceeded_507 / FailedEtagReleasesReservations / OverwriteShrink） | **已落地**（#575） |
...
| 8 | **P2：备份到远端卷** | 卷导出目标支持远端卷（federated/remote 写面）——本地 → 远端备份 | 卷导出本身未做（11.3） | **已落地（P1 引擎）**：`pkg/backup`（`Run(ctx, src, dst sync.FS, opts)`）——walk 源（递归 + Exclude glob）→ manifest 比对（`<backup>/manifest.json`，size+mtime 相同 skip）→ OpenRead → WriteFile（失败指数退避重试 100ms×2ⁿ）→ Verify 时对端 Stat 校验 size → 原子写 manifest（tmp+Rename）；worker 池并发（默认 4，峰值 ≤ Concurrent）；单文件失败隔离记 Failed+FileError；ctx 取消返回已拷贝部分 + Truncated；P2（路由/CLI/federated 写面装配/E2E）留后续 | 部分 |
| 9 | **P2：sclient 并发批量 + 进度条** | batch 命令并发执行（现逐行串行）+ 传输进度条（TUI） | **P1 已落地**：`cmd/sclient/batch_run.go` runBatchConcurrent（信号量限并发 + Index 槽位保序 + exec panic recover + SIGINT Skipped）+ `cmd/sclient/batch_cmd.go` batch 主命令重建（读文件逐行 → 并发执行 → printBatchResults 保序 → 任 FAIL 非 0 退出；--workers 默认 4/1 串行退化；delete/mkdir/rmdir/meta 子集）。P2（--progress 进度条）待后续 | 部分 |
...
| 10 | **P3：迁移向导** | 单机→多卷→联邦的自动化迁移脚本/向导（复用卷导出导入 + 镜像） | **已落地**：`sclient migrate export/import/mirror-config`（pkg/migrate：Manifest 清单 + Exporter 递归导出 + Importer 幂等/冲突/校验 + MirrorConfig YAML 片段；交互/纯脚本双模式） | 已落地 |
...
| A2 | `trash` | 服务端 /api/trash 有（list/restore/empty）——CLI 封装 | **已落地**（#571）：`sclient trash list|restore <trash_rel>|empty --yes`；restore 契约对齐 ?file= query（#608 修复） | **已落地** |
...
| | 11.8-A2/A3 trash/quota CLI | 可用性 | 1 人日 | 服务端已有，纯封装（**已落地** #571，restore 契约修复 #608） |
...
| **S5（长尾，按需）** | 11.9-① MCP server | 生态 | 5 人日 | 依赖 AI 客户端生态成熟；**已落地**（2026-09-25，见 11.9 已落地注） |
