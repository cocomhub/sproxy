...
| **P1：指标深化** | 传输层（TCP/WS/QUIC）指标入 `/metrics`；`/metrics` 加认证（`metrics_token` 或独立端口） | **已落地**（#497）：`metrics_token`（query/Bearer 常量时间比较，仅门 /metrics 零回归）+ xfer TCP 连接级指标（sproxy_xfer_tcp_conns/messages/bytes_*）；mux 流级已聚合。残余：独立指标端口 |
