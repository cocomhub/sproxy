...
| `audit.persist_dir` | string | (空) | 审计落盘目录（相对 `<默认卷根>`，如 `audit` → `<storage_root>/audit/audit.log`）；非空时 `RecordAudit` 同时 append JSON lines 落盘，**重启后 `/api/audit` 可查历史**（验收核心）；空 = 仅内存（默认零回归）。明文 JSON（审计行不含密钥/凭据）；写盘失败记日志并跳过（尽力而为，绝不阻断业务） |
| **分块上传** |  |  |  |
...
- **覆盖写**：覆盖旧内容先摘旧引用，再登记新 checksum 引用（旧内容若仍有其它引用则
  inode 保留）
- **硬链接限制**：依赖文件系统硬链接支持（本地卷均支持；FAT/exFAT 等不支持硬链接的
  文件系统上启用会报错——请确认存储卷文件系统支持硬链接）
