...
- **调用点清单是 grep 估计值，编译器才是权威**：各任务里列举的调用点文件，是按 `grep -rl <符号>` 数出来的，**会把只在注释/文档字符串里提及该符号的文件也算进去**（加导入会 `unused import` 编译失败），也可能遗漏。实施时**以 `go build ./...` 的报错为准**逐条加减，并在报告里写明**实际**数量与偏离原因。
  ...
  > 第三次实测（任务 3）：简报称 10，实际**生产 10 但名单不符**——漏了 **`cmd/sproxy/root.go`**（**跨 module**），多列了 `config_api.go`（零命中）。
  ...
  > 再一实测（任务 3）：简报称 10 个生产调用点，其中 `config_api.go` **零命中**（只调 `h.storageMgr` 的方法、从不写类型名）；但简报**漏了 11 个 `*_test.go`** 与**漏了 `cmd/sproxy/root.go`**（跨 module，`go build ./...` 在根 module 里看不见它，须另跑 `make build-all`）。
