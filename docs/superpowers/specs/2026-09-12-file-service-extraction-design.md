...
### 3.4 `pkg/client` 的评估结论（任务 10：**不拆分，零改动**）

**结论：`pkg/client` 保持单一包，不做任何搬迁。** 本片因此是**一次核对**，无代码改动。

**判据应用**：
- **P1（组织单位是文件）已满足**：24 个非测试 `.go`，**全部 `package client`**，包内**无 Go 子目录**；文件已按职责划分（`chunked`/`download_items`/`volume`/`version`/`share`/`sync`/`relay`/`mesh_refresh`…）。
- **无满足 P6①（可复用工具集合）或 P6②（真子领域）的可拆单元**。实测证据（均为"引用 `FileClient` 非导出字段"的处数，越大越说明耦合是结构性的）：`chunked.go` **27**、`relay.go` **11**、`accesskey.go` **11** ⇒ 拆出即须把大量内部字段导出或再造内部接口。`store.go`/`store_json.go` 的 `KVStore` **唯一使用者就是链框架** ⇒ **正是 P6 明文否掉的"某功能的处理器 + 它的存储"**。
- **反向依赖**：`pkg/tunnel/mesh` **import `pkg/client`** ⇒ 客户端不是其他域的"内部细节"，拆包还会牵动该方向。
- **佐证**：凡满足 P2/P6 的抽象（`accesskey`/`cloudfilename`/`sproxysig`/`pathguard`/`checksum`/`volume` 等）**早已在顶层包**——客户端里**没有**"该升为顶级却还埋着"的东西。
- **代价侧（门槛应当很高）**：`pkg/client` 是**公开 SDK**，拆包会改变**导入路径**（破坏性变更）；外部消费者按其"一个 `FileClient` 方法集"整体使用。

**附带登记（不在本工作范围）**：
- `pkg/client/format.go` 的 `KB/MB/GB/TB` 四个导出常量在**本仓**零引用，且与 `internal/size` 的 `KiB/MiB/GiB` 命名口径不一致。**不删除**——它是**公开 SDK 的导出符号**，"本仓零引用"**不足以**证明外部无人使用；删除属破坏性变更，须单独立项评估。
- `client.go`（1896 行）是"核心 + 文件面 + 传输栈 + 链入口"混居的最大文件，将来可在**同包内**重排（R34：不拆包），属重设计阶段。
