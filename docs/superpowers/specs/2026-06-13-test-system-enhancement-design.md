...
### 1.1 `pkg/testutil/testutil.go` — 包级测试工具集

包路径: `github.com/cocomhub/sproxy/pkg/testutil`
...
### 2026-06-13 v1 初始版本
- 创建 `pkg/testutil/`（TestKey, DiscardLogger, SHA256Hex, CaptureStdout, CaptureStderr）
- `internal/shortid/shortid_test.go` 5 个分支覆盖
- `handlers_test.go` parsePagination 7 场景
- `version_test.go` deleteVersionHandler 4 场景 + production bug fix
- `server_hub_test.go` Hub handler 5 场景
- `cmd_rune_test.go` sclient 子命令 12 个 RunE 测试
- 设计文档初版

### 2026-06-14 v2 完成
- `root_extra_test.go` SIGHUP/密钥/日志/端口占用
- `chunked_test.go` calcChunkSize/generateUploadID 边界测试
- `tunnel_extra_test.go` UpdateKey/panic/502/resolveKey
- `gzip_test.go` table-driven GzipMiddleware 5 场景
- `calcchunksize_fuzz_test.go` + `parsekey_fuzz_test.go` 模糊测试
- `e2e_extra_test.go` 5 个二进制级端到端测试
- 全量 staticcheck 归零
- Makefile test-packages/cover-html 目标
- 修复 test/e2e_test.go data race（sync.Once）
- 代码审查修复全局状态泄漏和死代码
