...
document.addEventListener('DOMContentLoaded', function() {
  ...
  // 必须在 DOMContentLoaded 内：顶部执行时 DOM 未就绪，按钮事件绑定不到。
  if (window.I18N) {
...
async function refreshList() {
  ...
  try {
    ...
    el.innerHTML = buildFileTableHtml(files, currentSubdir) + buildLoadMoreHtml(data.total, _currentOffset, _hasMore);
    updateBatchToolbar();
  } catch (e) {
...
function buildFileTableHtml(files, subdir) { return appRender.buildFileTableHtml(files, subdir); }
function buildFileRowHtml(fi, fullName) { return appRender.buildFileRowHtml(fi, fullName); }
// 参数化版：调用方传入 hasMore 与 offset（不再读模块状态 _hasMore/_currentOffset）。
function buildLoadMoreHtml(total, offset, hasMore) { return appRender.buildLoadMoreHtml(total, offset, hasMore); }
...
function configTableHtml(cfg) {
  // 配置表格 + 快速编辑面板：纯 HTML 渲染在 app-render.js（appRender），此处仅转发。
  const html = appRender.configTableHtml(cfg);
  // 快速编辑面板按钮绑定在 initDynamicEventDelegation 内完成（id 固定），静态 HTML 由
  // app-render 输出，无需在此额外绑定。
  return html;
}
...
function buildVersionTableHtml(versions, filename) { return appRender.buildVersionTableHtml(versions, filename); }
