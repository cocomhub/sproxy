...
async function tunnelRequest(method, urlPath, headersObj, bodyBytes) {
  ...
  if (headersObj) {
    for (const entry of Object.entries(headersObj)) {
      const k = entry[0];
      let v = entry[1];
  ...
  const respMetaLen = new DataView(respBytes.buffer, respBytes.byteOffset).getUint32(0, false);
  const respMetaEnc = respBytes.slice(4, 4 + respMetaLen);

  const respMetaPlain = await decryptAESGCM(key, respMetaEnc.slice(0, 12), respMetaEnc.slice(12));
  const respMeta = JSON.parse(new TextDecoder().decode(respMetaPlain));

  // 解密 body 帧
  const bodyStart = 4 + respMetaLen;
  const respBody = await decryptBodyFrames(key, respBytes, bodyStart);
...
async function decryptBodyFrames(key, respBytes, startOffset) {
  ...
  while (offset + 4 <= respBytes.length) {
    const bLen = new DataView(respBytes.buffer, respBytes.byteOffset + offset).getUint32(0, false);
    if (bLen === 0 || offset + 4 + bLen > respBytes.length) break;
    const bEnc = respBytes.slice(offset + 4, offset + 4 + bLen);
    if (bEnc.length >= 12) {
      const chunk = await decryptAESGCM(key, bEnc.slice(0, 12), bEnc.slice(12));
      chunks.push(chunk);
    }
    offset += 4 + bLen;
  }
...
async function tunnelDownloadStream(name) {
  ...
  const meta = { method: 'GET', url: '/download?filename=' + encodeURIComponent(name), headers: {} };
  const metaJSON = new TextEncoder().encode(JSON.stringify(meta));
  const metaIV = crypto.getRandomValues(new Uint8Array(12));
  const metaCT = await crypto.subtle.encrypt({ name: 'AES-GCM', iv: metaIV }, key, metaJSON);
  const metaFrame = new Uint8Array(4 + metaIV.length + metaCT.byteLength);
  ...
  const metaLenBytes = await readNBytes(reader, 4);
  const metaLen = new DataView(metaLenBytes.buffer).getUint32(0, false);
  const metaEnc = await readNBytes(reader, metaLen);
  const metaPlain = await decryptAESGCM(key, metaEnc.slice(0, 12), metaEnc.slice(12));
  const respMeta = JSON.parse(new TextDecoder().decode(metaPlain));
  ...
  while (true) {
    ...
    const frameData = remainder.slice(4, 4 + chunkLen);
    remainder = remainder.slice(4 + chunkLen);

    if (frameData.length >= 12) {
      const plain = await decryptAESGCM(key, frameData.slice(0, 12), frameData.slice(12));
      chunks.push(plain);
    }
  }
...
async function readNBytes(reader, n) {
  ...
  while (buf.length < n) {
    ...
    const tmp = new Uint8Array(buf.length + result.value.length);
    tmp.set(buf, 0);
...
async function fillBuffer(reader, remainder, n) {
  while (remainder.length < n) {
    ...
    const tmp = new Uint8Array(remainder.length + result.value.length);
    tmp.set(remainder, 0);
