...
- **单任务 / 批量**：`POST /api/cloud/download`、`POST /api/cloud/download/batch`，小文件同步返回、大文件异步下载。
...
相关配置键（见 `config.example.yaml`）：`cloud_max_concurrent`、`cloud_download_timeout`、`cloud_download_idle_timeout`、`cloud_max_retries`、`cloud_retry_delay`、`cloud_task_ttl`、`cloud_failed_task_ttl` 等。
