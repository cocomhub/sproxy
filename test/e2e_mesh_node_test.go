...
func startSClientMeshNode(t *testing.T, hubURL, nodeID, serviceSpec, ak, sk string, extraArgs ...string) func() {
	t.Helper()
	tmpDir := t.TempDir()
...
func startSClientMeshNodeObservable(t *testing.T, hubURL, nodeID, serviceSpec string, out *bytes.Buffer, ak, sk string, extraArgs ...string) func() {
	t.Helper()
	tmpDir := t.TempDir()
	binPath := e2eBinPath(t, "cmd/sclient")
	...
	waitNodeRegistered(t, hubURL, nodeID, ak, sk, out, newKillWaitCleanup(cmd))
	return newKillWaitCleanup(cmd)
}
...
func TestE2E_MeshNode_Discovery(t *testing.T) {
	...
	cleanupA := startSClientMeshNodeObservable(t, hubURL, "e2e-disc-a", "", &stderrA, ak, sk, "--discover-interval", "1s")
	defer cleanupA()
	cleanupB := startSClientMeshNode(t, hubURL, "e2e-disc-b", "", ak, sk, "--discover-interval", "1s")
	defer cleanupB()
...
func TestE2E_MeshNode_ServiceAccess(t *testing.T) {
	...
	cleanupAP := startSClientMeshNodeObservable(t, hubURL, "e2e-ap", "echo-ap:"+echoApAddr, &stderrAP, ak, sk,
		"--discover-interval", "1s", "--gateway-addr", "127.0.0.1:18085")
	defer cleanupAP()

	// node-svc（服务宿主 echo）：网关换端口避免同机冲突。
	cleanupSvc := startSClientMeshNode(t, hubURL, "e2e-svc", "echo:"+echoSvcAddr, ak, sk,
	...
	echoRoundTrip := func(service, gatewayAddr string) {
		listenAddr, meshCleanup := startSClientMeshConnect(t, hubURL, service, ak, sk,
			"--gateway", gatewayAddr)
		defer meshCleanup()
