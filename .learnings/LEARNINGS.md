...
### Resolution
- **Resolved**: 2026-06-19T22:00:00Z
- **PR/Commit**: PR #10 (`fix/path-injection-s2083`)
- **Notes**: 21 处替换总覆盖 7 个文件，并通过代码审查修复了 3 个额外发现后合并
...
## [LRN-20260619-BP75] subagent 生成的代码仍遗留 gofmt/errcheck lint 问题
...
### Summary
PR2 实现子代理完成后，`golangci-lint` 发现 2 个问题：`quic.go` 中 `qconn.CloseWithError` 返回值未检查（errcheck），`quic_internal_test.go` 中表测试字段未对齐（gofmt）。

### Details
虽然已有 BP57 记录了"子代理完成后主流程代理先跑 lint"，本次主流程代理已在 merge 前检查并修复了这 2 个问题，但问题本身反映了子代理的常见盲区：
1. **errcheck**: `qconn.CloseWithError(0, "stream failed")` — 子代理习惯性地忽略 Close 相关调用的错误返回
2. **gofmt**: 表测试声明时字段值未对齐（`"ipv6 with port"` 的 `name` 字段短于同表其他行），这在手动编辑时容易漏掉

两个问题都由 `golangci-lint` 捕获并被主流程修复后才提交。

### Suggested Action
子代理实现完后，主流程必须跑一次 `golangci-lint run ./...`（不限于 `make lint` 范围）确保格式化 + errcheck 都通过。对于 pre-commit hook 已自动跑 `go vet` + `gofmt` 但没跑 `golangci-lint` 的情况，在 commit 消息中验证时手动补充。

### Metadata
- Source: code_review
- Pattern-Key: tests.subagent_lint_check
- Recurrence-Count: 2 (see LRN-20260619-BP57)
- Related Files: `pkg/tunnel/xfer/ext/quic/quic.go`, `pkg/tunnel/xfer/ext/quic/quic_internal_test.go`

---

## [LRN-20260619-BP76] quic-go CloseWithError 的 errcheck 容易遗漏

**Logged**: 2026-06-19T22:05:00Z
**Priority**: low
**Status**: pending
**Area**: backend

### Summary
`qconn.CloseWithError(0, "stream failed")` 在 Go 中返回 error，但在 QUIC 连接建立后 stream 打开失败的场景中，调用 CloseWithError 主要是为了清理资源，错误返回通常不重要。

### Details
该行在 `quic.go:Dial()` 函数中，当 `OpenStreamSync` 失败时，为了清理 QUIC 连接而调用 `CloseWithError`。返回值中没有需要处理的信息（连接本就会因为 stream 失败而关闭）。修复：`_ = qconn.CloseWithError(...)`。

### Metadata
- Source: lint
- Pattern-Key: lint.errcheck.close_with_error
- Recurrence-Count: 1
- Related Files: `pkg/tunnel/xfer/ext/quic/quic.go`

---

## [LRN-20260619-BP77] quic 扩展包的独立 go.mod 导致 lint 需要显式指定路径

**Logged**: 2026-06-19T22:10:00Z
**Priority**: medium
**Status**: pending
**Area**: infra

### Summary
`pkg/tunnel/xfer/ext/quic/` 是一个独立的 Go module（自己的 `go.mod`），`golangci-lint run ./...` 从根目录执行不会覆盖子 module 的文件。PR2 中需要显式指定 `golangci-lint run ./pkg/tunnel/xfer/ext/quic/...` 才能发现该包的 lint 问题。

### Details
- `go build ./...` 从根目录即可覆盖（workspace 自动解析）
- `golangci-lint run ./...` 只跑主 module 的 lint，quic 子 module 需单独指定
- ext/ws 同理（也是独立 go.mod）
- 本次发现 quic 的 lint 问题是因为主流程在 `golangci-lint run ./...` 后手动加了 `./pkg/tunnel/xfer/ext/quic/...` 路径

### Suggested Action
涉及子 module（`ext/quic`、`ext/ws`）的 lint 检查必须显式指定路径：`golangci-lint run ./pkg/... ./cmd/... ./internal/... ./pkg/tunnel/xfer/ext/quic/... ./pkg/tunnel/xfer/ext/ws/...`。或者直接在 Makefile 中为每个独立 go.mod 的包单独配置 lint 目标。

### Metadata
- Source: tool_error
- Pattern-Key: infra.submodule_lint_scope
- Recurrence-Count: 1
- Related Files: `pkg/tunnel/xfer/ext/quic/go.mod`, `pkg/tunnel/xfer/ext/ws/go.mod`, `Makefile`

---

## [LRN-20260620-BP78] 超大函数重构策略：先提取公共辅助函数再替换

**Logged**: 2026-06-20T00:30:00Z
**Priority**: medium
**Status**: pending
**Area**: backend

### Summary
PR3 中重构 8 个 S3776 高认知复杂度 handler 时，采用"先提取公共辅助函数（`writeFileAtomically`、`resolveFilePath` 等），再用提取的函数替换各 handler 中重复代码"的策略，效果显著。

### Details
成功模式：
1. 先识别跨 handler 的重复模式（temp-file/hash/rename、ValidateFilePath+safePath、checksum 校验）
2. 提取为文件级函数或 `Handlers` 方法（不依赖 handler 具体逻辑）
3. 再逐个替换每个 handler 中的重复代码块
4. 每替换完一个验证一次编译

关键成果：
- `writeFileAtomically` 在 upload、uploadChunk、uploadComplete 三个 handler 中复用
- `resolveFilePath` 在 5 个 handler 中复用
- 8 个 handler 平均缩减 60% 行数
- 所有测试零修改零失败

这种"先公共后特化"的模式比"逐个 handler 提取"更有效，因为公共函数在第一个 handler 中提取后就可以在后续 handler 中立即复用，不需要反复调整。

### Metadata
- Source: workflow
- Pattern-Key: refactor.helper_first_then_replace
- Recurrence-Count: 1

---

## [LRN-20260620-BP79] handleFrame 巨型 switch → 分发表模式

**Logged**: 2026-06-20T01:00:00Z
**Priority**: low
**Status**: pending
**Area**: backend

### Summary
`pkg/tunnel/mux/mux.go` 的 `handleFrame` 包含 ~93 行的巨型 switch（8 种帧类型），每种类型含嵌套逻辑。提取为 `map[FrameType]frameHandler` 分发表后，主函数缩减为 ~15 行，每个帧类型处理在独立函数中。

### Details
重构模式：`巨型 switch → map[FrameType]frameHandler` 分发表
...
优点：
- 主函数 `handleFrame` 认知复杂度大幅降低
- 加新帧类型无需修改 switch，只需添加 map 条目
- 每个 handler 函数可独立测试
- 同包内文件级函数可直接访问 `Mux` 的私有字段

### Metadata
- Source: workflow
- Pattern-Key: refactor.switch_to_dispatch_map
- Recurrence-Count: 1
- Related Files: `pkg/tunnel/mux/mux.go`

---

## [LRN-20260620-BP80] stream.go 重构：结构体包装 + 薄委托

**Logged**: 2026-06-20T01:15:00Z
**Priority**: low
**Status**: pending
**Area**: backend

### Summary
`EncryptStreamWithChunkSize` 和 `DecryptStreamWithChunkSize` 各 ~57 行，缓冲区管理、nonce 生成、加密/解密、长度前缀读写交织在一起。提取为 `StreamEncryptor`/`StreamDecryptor` 结构体后，原始函数变为 ~3 行的薄委托。
...
// 重构后：3 行委托
func EncryptStreamWithChunkSize(key []byte, r io.Reader, w io.Writer, chunkSize int) (int64, error) {
    e, err := NewStreamEncryptor(key, chunkSize)
    if err != nil {
        return 0, err
    }
    return e.EncryptStream(r, w)
}
```

优点：
- 流加密/解密逻辑封装在结构体方法中，可独立调用（`EncryptChunk`/`DecryptChunk`）
- 保持向后兼容（原函数签名不变）
- 新增调用者可直接使用 `StreamEncryptor`/`StreamDecryptor`，无需重复缓冲区管理

### Metadata
- Source: workflow
- Pattern-Key: refactor.struct_wrapper_delegate
- Recurrence-Count: 1
- Related Files: `pkg/tunnel/stream.go`

---

## [LRN-20260620-BP81] ChunkedUploader 结构体封装并发+进度+重试
...
### Summary
`uploadChunks`（~181 行）中并发控制（信号量+WaitGroup）、进度追踪、重试逻辑交织在一起。提取 `ChunkedUploader` 结构体后，`uploadChunks` 缩减为 3 行委托。

### Details
提取结构体囊括了分块上传的全部状态：
- 信号量并发控制（`chan struct{}`）
- atomic 失败标记（`sync/atomic.Bool`）
- 重试循环（`for range maxRetries`）
- 进度追踪（`sync.Mutex` + `int64`）

方法拆分：
- `run(ctx, chunkIndices)` — 主循环
- `uploadChunk(ctx, index)` — 单块上传
- `openAndSeekChunk(index)` — 打开+寻道
- `buildChunkRequest(ctx, index)` — 构建 multipart 请求
- `sendChunkRequest(req)` — 发送+解析

### Metadata
- Source: workflow
- Pattern-Key: refactor.state_struct_concurrency
- Recurrence-Count: 1
- Related Files: `pkg/client/chunked.go`

---

## [LRN-20260620-BP82] 纯重构不应修改测试文件 — 但需要验证错误消息兼容性

**Logged**: 2026-06-20T01:45:00Z
**Priority**: medium
**Status**: pending
**Area**: tests

### Summary
PR3 的 `uploadInit` 重构中，辅助函数提取后 hex checksum 校验失败的错误消息发生了细微变化（从 `"file_checksum 不是有效的 hex"` 变为不同格式），导致测试失败。需要微调错误消息以保持测试兼容。

### Details
重构过程中发现：
- 提取的校验函数返回的错误消息与原来测试断言的消息不完全一致
- 子代理在实现中发现了此问题并主动修正了错误消息以匹配测试断言
- 这是纯重构的原则性要求：**功能不变，包括错误消息**

教训：
- 即使标记为"纯重构/未修改测试文件"，提取辅助函数时的错误消息也必须精确匹配原版
- 可以先跑一次测试确认原版消息，再重构
- 如果提取时消息必然变化，需要同步更新测试断言

### Metadata
- Source: code_review
- Pattern-Key: tests.refactor_message_compatibility
- Recurrence-Count: 1
- Related Files: `pkg/server/chunked_upload.go`

---

## [LRN-20260620-BP83] CLI 函数重构：顺序阶段提取模式

**Logged**: 2026-06-20T02:00:00Z
**Priority**: low
**Status**: pending
**Area**: backend

### Summary
`runServer`（198 行，11 个阶段）和 `tunnelRequest`（128 行，11 个阶段）都是按顺序执行阶段的 CLI 初始化/请求函数。提取策略：每个阶段一个独立函数，主函数仅保留编排逻辑。

### Details
`runServer` 的 11 个阶段有明显的阶段边界（配置加载 → 日志初始化 → 路由注册 → 信号处理 → 监听），每个阶段可以独立提取：
- `buildServerConfig` — 阶段 2-3（配置加载）
- `createHTTPServer` — 阶段 7（Server 创建）
- `startTLSListener` — 阶段 10（TLS 监听）
- `runSignalHandler` — 阶段 8-9（信号处理 goroutine）

关键设计决策：
- 信号处理 goroutine 的 `testSignalCh` 包级变量保留在 `runSignalHandler` 内部
- `runSignalHandler` 返回 `shutdownDone` channel 供主函数等待
- 提取的函数直接捕获包级变量，不需要额外传参

`tunnelRequest` 同理提取了 `resolveOutputPath` 和 `writeWithProgress`。

### Metadata
- Source: workflow
- Pattern-Key: refactor.cli_phase_extraction
- Recurrence-Count: 1
- Related Files: `cmd/sproxy/root.go`, `cmd/sclient/tunnel.go`

---

## [LRN-20260620-BP84] 手动编辑测试文件修复 lint 极易出错 — 优先 git checkout 回滚

**Logged**: 2026-06-20T02:15:00Z
**Priority**: medium
**Status**: pending
**Area**: infra

### Summary
PR5 中尝试手动编辑 `cmd/sproxy/root_extra_test.go` 修复 `gocritic` 的 `elseif` lint 告警时，连续 5 次 Edit 操作都导致语法错误（括号不匹配、缩进错乱），最终靠 `git checkout -- file` 回滚。该告警是 PR5 之前就存在的，不是本次引入。

### Details
`root_extra_test.go:294` 的 `} else {` → `} else if ... {` 转换看似简单，但测试函数内部有多层嵌套（`for range` → `t.Run` → `if format == "json" {` → `if output[0] != '{'` 等），缩进层级复杂。每次 Edit 操作后缩进或多括号都出错，`gofmt -w` 也无法修复（语法错误导致 gofmt 拒绝格式化）。

教训：
- 对于已有的 lint 告警（非本次 PR 引入的），不要试图在改 PR 中一并修复
- 测试文件的嵌套结构通常比生产代码更深，手动编辑风险更高
- 当连续 2 次 Edit 出错时，应放弃修改而不是继续
- `git checkout -- file` 安全回滚

### Metadata
- Source: tool_error
- Pattern-Key: tests.lint_fix_rollback
- Recurrence-Count: 1
- Related Files: `cmd/sproxy/root_extra_test.go`

---

## [LRN-20260620-BP85] subagent 提交的代码修复后主流程仍需跑全局 lint

**Logged**: 2026-06-20T02:30:00Z
**Privacy**: medium
**Stat**: pending
**Area**: infra

### Summary
PR5 子代理提交后，`golangci-lint run ./cmd/sproxy/...` 和 `./cmd/sclient/...` 发现 10 个 lint 问题。其中 9 个是已有问题（relay.go/diag.go/cd.go 等），1 个是 PR5 新引入的（`os.Chtimes` errcheck）。主流程修复了新引入的 1 个，已存在的 9 个未动。

### Details
sclient 包 lint 检查结果为 9 个问题：
- `relay.go`: io.Copy errcheck（已有）、SSRF G704（已有）、govet shadow（已有）、whitespace（已有）
- `cd.go`: gocritic ifElseChain（已有）
- `diag.go`: govet shadow（已有）
- `cmd_rune_test.go`: SA9003 empty branch（已有）
- `tunnel.go`: os.Chtimes errcheck（**新引入**，已修复 ✅）

sproxy 包：
- `root_extra_test.go`: gocritic elseif（已有）

### Suggested Action
主流程代理在 subagent 提交后必须跑 `golangci-lint run ./cmd/...`，逐一确认每个问题是已有的还是新引入的。新引入的必须修，已有的标记为 baseline 不碰。

### Metadata
- Source: code_review
- Pattern-Key: tests.subagent_lint_check
- Recurrence-Count: 3 (BP57 + BP75 + BP85)
- Related Files: `cmd/sclient/tunnel.go`, `cmd/sclient/relay.go`
